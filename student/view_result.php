...
$quizTitle = $quizData ? $quizData['title'] : 'Quiz';

// Get course name
$courseSql = "SELECT c.title FROM courses c JOIN quizzes q ON c.id = q.course_id WHERE q.id = :quiz_id";
$courseStmt = $conn->prepare($courseSql);
$courseStmt->execute(['quiz_id' => $quiz_id]);
$courseData = $courseStmt->fetch(PDO::FETCH_ASSOC);
$courseTitle = $courseData ? $courseData['title'] : 'Course';

if (!$result) {
    $resultMessage = "<p class='error-message'>No results found for this quiz.</p>";
} else {
    $resultMessage = "
        <div class='result-item'>
            <div class='result-icon-container'>
                <svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
                    <g clip-path='url(#clip0_result)'>
            ...
            <div class='result-details'>
                <p class='result-text'>Score: {$result['score']} / {$result['total_questions']}</p>
                <p class='result-text'>Percentage: {$result['percentage']}%</p>
            </div>
        </div>
    ";
}
    ...
    <div class="app-container">
        ...
        <main class="main-content">
            ...
            <section class="result-container">
                <h2 class="course-title"><?php echo htmlspecialchars($courseTitle); ?></h2>
                <h3 class="quiz-title"><?php echo htmlspecialchars($quizTitle); ?></h3>
                <h4 class="result-heading">Quiz Results</h4>
