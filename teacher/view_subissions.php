    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!isset($_POST['submission_id']) || !is_numeric($_POST['submission_id'])) {
            ...
            if ($stmt->execute()) {
                ...
                $error_message = "Error grading assignment.";
            }
        }
    }
...
<head>
    ...
    <title>View Submissions - Online Smart Class</title>
                                ...
                                <li class="submission-item">
                                    <div class="submission-header">
                                        <div>
                                            ...
                                            <p class="assignment-title">Assignment: <?php echo htmlspecialchars($row['title']); ?></p>
                                        </div>

                                        <!-- ✅ Validate and provide download link -->
                                        <?php if (!empty($row['file_path']) && file_exists("../uploads/" . $row['file_path'])): ?>
                                            <a href="../uploads/<?php echo htmlspecialchars($row['file_path']); ?>" download class="download-link">Download Submission</a>
                                        <?php else: ?>
                                            <span class="file-unavailable">File not available</span>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <form method="POST" class="grading-form">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="grade" class="form-input" required value="<?php echo isset($row['grade']) ? htmlspecialchars($row['grade']) : ''; ?>">
                                        </div>

                                        <div class="form-group">
                                            <label class="form-label">Feedback:</label>
                                            <textarea name="feedback" class="form-textarea"><?php echo isset($row['feedback']) ? htmlspecialchars($row['feedback']) : ''; ?></textarea>
                                        </div>

                                        <button type="submit" class="form-button">Submit Grade</button>
                                    </form>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    <?php else: ?>
                        <p class="no-submissions">No submissions available.</p>
                    <?php endif; ?>
